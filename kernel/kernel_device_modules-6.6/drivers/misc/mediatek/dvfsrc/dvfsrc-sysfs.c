...
int geas_update_emi_params(struct emi_params *data)
{
	int ret = 1;

	if (dvfsrc_ptr && data->opp >= 0 && data->opp < dvfsrc_ptr->opp_desc->num_dram_opp) {
		mtk_dvfsrc_set_ceiling_freq(CEILING_SYSFS, data->opp);
		ret = 0;

		pr_err("%s, set dvfsrc_ceiling_opp = %d", __func__, data->opp);
	}

	return ret;
}
EXPORT_SYMBOL(geas_update_emi_params);
#endif
