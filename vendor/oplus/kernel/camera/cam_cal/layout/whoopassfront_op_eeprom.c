...
#define PFX "CAM_CAL_WHOOPASSC1FRONT"
...
static unsigned int whoopassfront_layout_check(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int sensorID, unsigned int *_cfg);
static unsigned int do_single_lsc_whoopassc1front(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_whoopassc1front(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_whoopassc1front(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_whoopassc1front(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x0176010a, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000000, 0x00000002, do_part_number},
		{0x00000001, 0x00000C00, 0x0000074C, do_single_lsc_whoopassc1front},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_whoopassc1front}, //Start address, block size is useless
		{0x00000001, 0x00001400, 0x00000A3A, do_pdaf_whoopassc1front},
		{0x00000000, 0x00000000, 0x00000000, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_whoopassc1front}
	}
};

static unsigned int sensor_id_list[] = {WHOOPASSC1FRONT_SENSOR_ID, WHOOPASSS1FRONT_SENSOR_ID, WHOOPASSC2FRONT_SENSOR_ID, WHOOPASSS2FRONT_SENSOR_ID};
struct STRUCT_CAM_CAL_CONFIG_STRUCT whoopassfront_op_eeprom = {
	.name = "whoopassfront_op_eeprom",
	.check_layout_function = whoopassfront_layout_check,
...
#define WHOOPASSC1FRONT_HVBIN_PDAF_PROC1_SIZE  (496)
#define WHOOPASSC1FRONT_HVBIN_PDAF_PROC2_SIZE  (1004)
#define WHOOPASSC1FRONT_HVBIN_PDAF_PROC1_ADDR  (0x1400)
#define WHOOPASSC1FRONT_HVBIN_PDAF_PROC2_ADDR  (0x1600)

unsigned int do_pdaf_whoopassc1front(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = WHOOPASSC1FRONT_HVBIN_PDAF_PROC1_SIZE + WHOOPASSC1FRONT_HVBIN_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			WHOOPASSC1FRONT_HVBIN_PDAF_PROC1_ADDR + WHOOPASSC1FRONT_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			WHOOPASSC1FRONT_HVBIN_PDAF_PROC2_ADDR + WHOOPASSC1FRONT_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			WHOOPASSC1FRONT_HVBIN_PDAF_PROC2_ADDR, WHOOPASSC1FRONT_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[WHOOPASSC1FRONT_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
