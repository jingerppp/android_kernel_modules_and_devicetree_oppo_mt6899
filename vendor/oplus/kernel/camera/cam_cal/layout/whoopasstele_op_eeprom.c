...
#define PFX "CAM_CAL_WHOOPASSC1TELE"
...
static unsigned int whoopasstele_layout_check(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int sensorID, unsigned int *_cfg);
static unsigned int do_single_lsc_whoopassc1tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_whoopassc1tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_whoopassc1tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_whoopassc1tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static int do_ois_whoopassc1tele(struct EEPROM_DRV_FD_DATA *pdata, struct ois_cal_data *data);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x00c6010d, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000B00, 0x0000074C, do_single_lsc_whoopassc1tele},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_whoopassc1tele}, //Start address, block size is useless
		{0x00000001, 0x00001300, 0x00000A3A, do_pdaf_whoopassc1tele},
		{0x00000000, 0x00000000, 0x00000000, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_whoopassc1tele}
	}
};

static unsigned int sensor_id_list[] = {WHOOPASSC1TELE_SENSOR_ID, WHOOPASSS1TELE_SENSOR_ID, WHOOPASSC2TELE_SENSOR_ID, WHOOPASSS2TELE_SENSOR_ID};
struct STRUCT_CAM_CAL_CONFIG_STRUCT whoopasstele_op_eeprom = {
	.name = "whoopasstele_op_eeprom",
	.check_layout_function = whoopasstele_layout_check,
	...
	.preload_size = 0x8000,
	.has_stored_data = 1,
	.parsing_ois_cal_data_from_preload = do_ois_whoopassc1tele,
};

unsigned int whoopasstele_layout_check(struct EEPROM_DRV_FD_DATA *pdata,
  				unsigned int sensorID, unsigned int *_cfg)
{
	...
	for (i = 0; i < ARRAY_SIZE(sensor_id_list); i++) {
		if(sensor_id_list[i] == sensorID) {
			debug_log("%s sensor_id matched 0x%x\n", cfg->name, sensorID);
			cfg->sensor_id = sensor_id_list[i];
			break;
		}
	}
	if (i ==  ARRAY_SIZE(sensor_id_list)) {
...
#define WHOOPASSC1TELE_HVBIN_PDAF_PROC1_SIZE  (496)
#define WHOOPASSC1TELE_HVBIN_PDAF_PROC2_SIZE  (1004)
#define WHOOPASSC1TELE_HVBIN_PDAF_PROC1_ADDR  (0x1300)
#define WHOOPASSC1TELE_HVBIN_PDAF_PROC2_ADDR  (0x1500)

unsigned int do_pdaf_whoopassc1tele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = WHOOPASSC1TELE_HVBIN_PDAF_PROC1_SIZE + WHOOPASSC1TELE_HVBIN_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			WHOOPASSC1TELE_HVBIN_PDAF_PROC1_ADDR + WHOOPASSC1TELE_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			WHOOPASSC1TELE_HVBIN_PDAF_PROC2_ADDR + WHOOPASSC1TELE_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			WHOOPASSC1TELE_HVBIN_PDAF_PROC2_ADDR, WHOOPASSC1TELE_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[WHOOPASSC1TELE_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (0x1 & AWBAFConfig) {
		...
		if (read_data_size > 0)	{
			...
			debug_log("Light source calibration 5000K value R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
		...
		/* AWB Unit Gain (5000K) */
		pr_debug_if(dump_enable, "5000K AWB\n");
		awb_offset = 0x20;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		pr_debug("AWB Calibration @5000K\n");
