...
#define PFX "CAM_CAL_WHOOPASSC1WIDE"
...
static unsigned int whoopasswide_layout_check(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int sensorID, unsigned int *_cfg);
static unsigned int do_single_lsc_whoopassc1wide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_whoopassc1wide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_whoopassc1wide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_whoopassc1wide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01ab0115, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000B00, 0x0000074C, do_single_lsc_whoopassc1wide},
		{0x00000001, 0x00000020, 0x0000000E, do_2a_gain_whoopassc1wide},  //Start address, block size is useless
		{0x00000001, 0x00001300, 0x00000A3A, do_pdaf_whoopassc1wide},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00004000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_whoopassc1wide}
	}
};

static unsigned int header_id_list[] = {0x01ab0115, 0x01ab0136};
static unsigned int sensor_id_list[] = {WHOOPASSC1WIDE_SENSOR_ID, WHOOPASSS1WIDE_SENSOR_ID, WHOOPASSC2WIDE_SENSOR_ID, WHOOPASSS2WIDE_SENSOR_ID};
struct STRUCT_CAM_CAL_CONFIG_STRUCT whoopasswide_op_eeprom = {
	.name = "whoopasswide_op_eeprom",
	.check_layout_function = whoopasswide_layout_check,
  				...
  				unsigned int sensorID, unsigned int *_cfg)
{
	...
	for (i = 0; i < ARRAY_SIZE(header_id_list); i++) {
		if(header_id_list[i] == check_id) {
			debug_log("header_id matched 0x%08x 0x%08x\n",
				check_id, header_id_list[i]);
			result = CAM_CAL_ERR_NO_ERR;
			break;
		}
	}
	if (i ==  ARRAY_SIZE(header_id_list)) {
...
#define WHOOPASSC1WIDE_HVBIN_PDAF_PROC1_SIZE  (496)
#define WHOOPASSC1WIDE_HVBIN_PDAF_PROC2_SIZE  (1004)
#define WHOOPASSC1WIDE_HVBIN_PDAF_PROC1_ADDR  (0x1300)
#define WHOOPASSC1WIDE_HVBIN_PDAF_PROC2_ADDR  (0x1500)

unsigned int do_pdaf_whoopassc1wide(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = WHOOPASSC1WIDE_HVBIN_PDAF_PROC1_SIZE + WHOOPASSC1WIDE_HVBIN_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			WHOOPASSC1WIDE_HVBIN_PDAF_PROC1_ADDR + WHOOPASSC1WIDE_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			WHOOPASSC1WIDE_HVBIN_PDAF_PROC2_ADDR + WHOOPASSC1WIDE_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			WHOOPASSC1WIDE_HVBIN_PDAF_PROC2_ADDR, WHOOPASSC1WIDE_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[WHOOPASSC1WIDE_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
